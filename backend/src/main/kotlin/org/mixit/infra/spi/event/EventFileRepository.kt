...
class EventFileRepository(
    ...
    override fun findAll(): List<Event> = staticFileRepository.findAll().map { it.toEvent() }

    override fun findOne(id: String): Event? = staticFileRepository.findAll().firstOrNull { it.id == id }?.toEvent()
